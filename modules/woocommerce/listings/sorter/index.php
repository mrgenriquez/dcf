...
if( !class_exists( 'NammOrganic_Woo_Listing_Sorter' ) ) {

    class NammOrganic_Woo_Listing_Sorter {
        ...
        function __construct() {
            ...
            /* Sorter CSS */
                add_filter( 'namm_organic_woo_archive_css', array( $this, 'woo_sorter_css'), 10, 1 );

            /* Sorter js */
                add_filter( 'namm_organic_woo_archive_js', array( $this, 'woo_sorter_js'), 10, 1 );
        ...
        */
            ...
            function module_dir_path() {
                ...
                if( namm_organic_is_file_in_theme( __FILE__ ) ) {
                    return NAMM_ORGANIC_MODULE_DIR . '/woocommerce/listings/sorter/';
                } else {
            ...
            function module_dir_url() {
                ...
                if( namm_organic_is_file_in_theme( __FILE__ ) ) {
                    return NAMM_ORGANIC_MODULE_URI . '/woocommerce/listings/sorter/';
                } else {
        ...
        */
            function woo_sorter_settings() {
                ...
                $settings = apply_filters( 'namm_organic_woo_sorter_settings', array () );
        ...
        */
            ...
            function woo_display_mode_options() {
                ...
                if( is_shop() || is_product_category() || is_product_tag() ) {
                    ...
                    $column = apply_filters( 'loop_shop_columns', 5 );
                    ...
                    echo '<div class="product-layout-controller '.esc_attr($grid_controller_class).'">';
                        echo '<ul class="product-change-layout">';
                            echo '<li class="hidden"><span data-column="1" class="'.esc_attr($one_column_class).'">'.esc_html__('1', 'namm-organic').'</span></li>';
                            echo '<li><span data-column="2" class="'.esc_attr($two_column_class).'">'.esc_html__('2', 'namm-organic').'</span></li>';
                            echo '<li><span data-column="3" class="'.esc_attr($three_column_class).'">'.esc_html__('3', 'namm-organic').'</span></li>';
                            echo '<li><span data-column="4" class="'.esc_attr($four_column_class).'">'.esc_html__('4', 'namm-organic').'</span></li>';
                            echo '<li><span data-column="5" class="'.esc_attr($five_column_class).'">'.esc_html__('5', 'namm-organic').'</span></li>';
                        echo '</ul>';
                    echo '</div>';
                    ...
                    echo '<div class="product-list-options-controller '.esc_attr($list_controller_class).'">';
                        echo '<ul class="product-list-options">';
                            echo '<li><span data-list-option="left-thumb" class="'.esc_attr($left_thumb_class).'">'.esc_html__('Left Thumb', 'namm-organic').'</span></li>';
                            echo '<li><span data-list-option="right-thumb" class="'.esc_attr($right_thumb_class).'">'.esc_html__('Right Thumb', 'namm-organic').'</span></li>';
                        echo '</ul>';
                    echo '</div>';
                ...
                if( is_shop() || is_product_category() || is_product_tag() ) {
                    ...
                    echo '<div class="product-display-controller">';
                        echo '<ul class="product-change-display">';
                            echo '<li><span data-display="grid" class="'.esc_attr($grid_display_mode_class).'">'.esc_html__('Grid', 'namm-organic').'</span></li>';
                            echo '<li><span data-display="list" class="'.esc_attr($list_display_mode_class).'">'.esc_html__('List', 'namm-organic').'</span></li>';
                        echo '</ul>';
                    echo '</div>';
            ...
            function woo_filters_widget_area() {
                echo '<div class="product-loop-filters-area-group">';
                    echo '<div class="product-loop-filters-area-title">'.esc_html__('Filters', 'namm-organic').'</div>';
                echo '</div>';
            }
            ...
            function woo_sorting_custom_extension() {
                echo '<div class="product-loop-filters-area-content hide">';
                    echo '<div class="product-loop-filters-area-content-inner">';
                        dynamic_sidebar( 'namm_organic-shop-filters' );
                    echo '</div>';
                echo '</div>';
            }
...
if( !function_exists('namm_organic_woo_listing_sorter') ) {
	function namm_organic_woo_listing_sorter() {
		return NammOrganic_Woo_Listing_Sorter::instance();
	}
}

namm_organic_woo_listing_sorter();