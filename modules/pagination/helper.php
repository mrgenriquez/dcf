<?php
add_action( 'namm_organic_after_main_css', 'pagination_style' );
function pagination_style() {
    wp_enqueue_style( 'namm_organic-pagination', get_theme_file_uri('/modules/pagination/assets/css/pagination.css'), false, NAMM_ORGANIC_THEME_VERSION, 'all');
}
...
if( !function_exists( 'after_single_page_content_wp_link_pages' ) ) {
    ...
    function after_single_page_content_wp_link_pages() {
        ...
        edit_post_link( esc_html__( ' Edit ','namm-organic' ) );
    }

    add_action( 'namm_organic_after_single_page_content', 'after_single_page_content_wp_link_pages' );
}

if( !function_exists( 'namm_organic_pagination' ) ) {
    function namm_organic_pagination( $query = false, $load_more = false ) {
        ...
        $end_size = 1;
        $mid_size = 2;
        #$show_all = namm_organic_get_option( 'showall-pagination' );
        $dots = false;
                ...
                if( $paged < $total ){
                    $output .= '<div class="column one pager_wrapper pager_lm">';
                        $output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
                            ...
                            $output .= '<span class="button_label">'. esc_html__('Load more', 'namm-organic') .'</span>';
                        $output .= '</a>';
                    $output .= '</div>';
                }
